...
public final class CopyChannel implements LxTool {
  ...
  public String description() {
    return "Copy a whole channel into the mixer — patterns, their effects and nested racks, "
        ...
        + "MIDI mappings, snapshot views and clip automation held by other channels stay "
        + "on the source; they are listed in the response's unreplicatedWiring array (kind, "
        + "scope, sourcePath, targetPath, plus type/channel/number for MIDI mappings). The "
        + "channel's own clips travel with it and are not listed. "
        ...
        + "to append. An index that would land inside an existing group and split it is "
        + "rejected with invalid_argument — copy outside the group, then group_channels. "
        + "Inserting shifts the 1-based paths of later channels — re-list rather "
  ...
  public Result<Map<String, Object>> handle(LX lx, Map<String, Object> args) {
    ...
    Map<String, Object> payload = ChannelPayload.channel(channel);
