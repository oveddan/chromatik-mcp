...
public final class WireModulator implements LxTool {
  ...
  public String description() {
    return "Wire a continuous modulation from a source parameter (e.g. a macro knob's "
        + "macro1) onto a target parameter. The target must be a compound parameter (most "
        + "device/mixer knobs are). Scope is inferred from the source — a knob inside a "
        + "device chain wires within that device; pass scope explicitly to override. "
        + "Adjust depth/direction afterwards via set_parameter on the returned rangePath / "
        + "polarityPath. Undoable in Chromatik with Cmd-Z. Caution: a wiring LX rejects "
        + "(circular dependency) clears Chromatik's undo history.";
  }
  ...
  public Result<Map<String, Object>> handle(LX lx, Map<String, Object> args) {
    ...
    LXCompoundModulation modulation =
        Modulators.wireModulation(lx, engine, normalizedSource, compoundTarget);
