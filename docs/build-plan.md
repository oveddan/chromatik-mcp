# chromatik-mcp — build plan (core tool surface shipped; follow-ups in progress)
...
- [x] **Issue #201** — `move_channel` reorders plain channels or whole group blocks by an absolute post-removal mixer index, preserves group membership, reports canonical-path changes, and is undoable through `LXCommand.Mixer.DropChannel`.
...
- [x] **PR-6** — Install docs + multi-agent usage examples + README rewrite — merged via [#24](https://github.com/oveddan/chromatik-mcp/pull/24); `docs/install.md`, `docs/usage-examples.md`, README quick start, and MIT license (capabilities base landed via [#19](https://github.com/oveddan/chromatik-mcp/pull/19))
...
- [x] **Composition tool surface** — 26 tools over the LX 1.2.2 arrange timeline (`/lx/timeline/composition` + grid clips), merged via [#168](https://github.com/oveddan/chromatik-mcp/pull/168): transport/markers, locators, lane lifecycle, paged event reads, automation editing/ranges, audio/text lanes, and record arm. Contracts: cursor objects everywhere (echo the engine read-back, never the request — setters clamp silently), positional lane/event addresses, `isIgnored`-command no-ops detected by state read, and explicit not-undoable descriptions. Cursor, pagination, and 1-indexed locator conventions are recorded in `docs/tool-conventions.md`.
- [x] **MIDI template discovery and creation** — `list_midi_templates` + undoable `add_midi_template`, merged via [#192](https://github.com/oveddan/chromatik-mcp/pull/192); templates expose named hardware controls at canonical paths for parameter discovery and modulation/trigger wiring
- [x] **Undo/redo command history** — issue [#198](https://github.com/oveddan/chromatik-mcp/issues/198), merged via [#199](https://github.com/oveddan/chromatik-mcp/pull/199); one-step shared linear history with affected-command and next-history reporting, explicit empty no-ops, verified failure/stack-clear diagnostics, and top-level-only batch safety. The history is engine-global, not session-scoped — responses name the command that moved so callers don't assume
- [x] **PatternRack containers in `add_pattern`** — merged via [#197](https://github.com/oveddan/chromatik-mcp/pull/197); `containerPath` accepts a PatternRack as well as a channel, so nested pattern lists are authorable
- [x] **Camera control** — issue [#222](https://github.com/oveddan/chromatik-mcp/issues/222); `get_camera`/`set_camera`/`save_camera`/`list_cameras`/`recall_camera`/`remove_camera` plus a `camera` argument on `get_frame`. One viewpoint, shared: `chromatikmcp.domain.Cameras` reproduces LX's own orbit convention (`UI3dContext.Camera` — eye = target + (r·cosφ·sinθ, r·sinφ, −r·cosφ·cosθ), phi clamped ±89, fov 15-150), the UI companion binds it to `LXStudio.UI.preview` so a move lands on the preview a person is watching, and `CameraProjection` reproduces LX's perspective/orthographic framing so the offline raster agrees with the screen. Named angles persist via `lx.registerExternal` (project `externals`), not session memory. Camera moves are deliberately not command-backed (LX has no camera `LXCommand`, and a viewpoint isn't engine state Cmd-Z should walk back)
- [x] **Channel grouping** — `group_channels` / `ungroup_channel` / `ungroup_channels`, merged via [#204](https://github.com/oveddan/chromatik-mcp/pull/204); group creation is the deliberate not-undoable exception (LX has no explicit-list command), both ungroup operations are undoable, and every grouping mutation reports `oscChanges` because the flat mixer paths reindex
...
- [ ] *Follow-up* — status.json lifecycle: rewrite `projectPath` when the open project changes (LX listener), and decide a multi-instance discovery contract. The file already updates connection state, records `connected: false` on clean shutdown, and documents the pid-liveness check; it remains on disk after shutdown, while two Chromatik instances still overwrite each other —
...
- [x] *Resolved (2026-06-10)* — `save_project` persistence tool deferred to **Phase 2**: v1 mutations stay in-memory; the user saves manually in Chromatik — **reversed (2026-07-25, [#154](https://github.com/oveddan/chromatik-mcp/issues/154))**: structure work done over MCP is unrecoverable on restart, and the `syncModelFile` write-through hazard needs a save-as escape hatch, not just visibility. `save_project`/`save_model` shipped via [#167](https://github.com/oveddan/chromatik-mcp/pull/167) —
...
- **PR-9 fan-out** — surface unexposed LX functionality (gap sweep 2026-07-17: chromatik-mcp uses 12 of ~110 `LXCommand` classes; these slices close the highest-value gaps, all parallel-safe). All five merged 2026-07-17:
  ...
  - **PR-9d** (#58) — mixer performance surface: `list_channels` gains crossfader + A/B crossfade groups, cue/aux, per-channel blend mode, auto-mute, pattern auto-cycle/transition settings (paths included). Grouping was initially skipped because `LXCommand.Mixer.GroupSelectedChannels` sources channels from UI selection state; #200 later shipped explicit-list grouping through `LXMixerEngine.addGroup(List)` (not undoable) plus command-backed member/group ungroup tools (undoable).
