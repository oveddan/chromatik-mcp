...
- [~] **PR-5b** — Channels / patterns / effects tools — branch `pr-5b-channel-tools`; nine tools (`add/remove_channel`, `add/remove/activate/move_pattern`, `add/remove/move_effect`) via `Channels` primitives; pre-checked hazards: locked effects and out-of-range indices (LX throws inside perform → undo-stack wipe), BLEND-mode `activate_pattern` (goPattern silently no-ops outside PLAYLIST); pattern primitives use `getEngine()` so PatternRack-hosted patterns resolve; effect containers = channel/master/pattern
