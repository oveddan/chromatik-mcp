...
- [~] **Composition tool surface** — 26 tools over the LX 1.2.2 arrange timeline (`/lx/timeline/composition` + grid clips): foundation (`Resolve` timeline branch, lane-aware canonical paths, `domain/Cursors` wire codec, `get_composition`/`list_clip_lanes`) + 7 parallel families — F1 transport/markers (`get_clip`, `launch_clip`, `stop_clip`, `set_clip_marker`), F2 locators (5 tools, 1-indexed addressing), F3 lane lifecycle (idempotent `add_clip_lane`, `removable` guard, constrained `move_clip_lane`, `set_clip_lane_visible`), F4 event read/insert (`get_clip_lane` paged window, `add_automation_point`), F5 `set_automation_point` (cursor/value/curve/shape, `atCursor` guard), F6 removal + ranges (`remove_automation_point`, `remove_clip_range`, `collapse_clip_range`), F7 audio/text/arm (`add_audio_lane`, `add_notes_lane`, `add_clip_note`, `set_clip_note`, `set_composition_arm`). Contracts: cursor objects everywhere (echo the engine read-back, never the request — setters clamp silently), positional lane/event addresses, `isIgnored`-command no-ops detected by state read, not-undoable tools say so. 93 tools total; integrated on `composition-tools`, review pending, owner merges locally. Docs-pass follow-up: the `tool-conventions.md` amendment (cursor wire format, pagination envelope) must ALSO codify the locator deviation — locators are 1-indexed everywhere (payload `index` and index-as-input-address on remove/move/go_locator), a deliberate exception to the "payload index fields are 0-based" rule that lanes/events still follow —
