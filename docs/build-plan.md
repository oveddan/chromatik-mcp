...
- [~] **PR-5d** — MIDI mapping tool — branch `pr-5d-midi-mapping`; `add_midi_mapping {target, type: note|cc, channel 1-16, number 0-127}` (declarative args synthesized into an `LXShortMessage` per the mapping row; note-on carries MAX_VELOCITY so it isn't read as note-off), `list_midi_mappings`, `remove_midi_mapping {index}`. **Deviation:** `LXMidiMapping` is not an `LXComponent` (no path/id), so mappings are addressed by 0-based list index with the usual staleness caveat. Pre-checks before command construction: channel/number ranges and `isMappable()` (LX's `LXMidiMapping.create` doesn't check it — it's a UI-level filter, and mapping an internal config param would otherwise silently succeed)
- [x] **PR-6** — Install docs + multi-agent usage examples + README rewrite — merged via [#24](https://github.com/oveddan/lx-mcp/pull/24); `docs/install.md` (build → `mvn install -Pinstall` → enable in Preferences → status.json discovery with pid-liveness caveat → client connection incl. Claude Code one-liner → troubleshooting), `docs/usage-examples.md` (five agent flows: understand / build structure / chain effects / macro-map with OSC / multi-agent patterns), README quick-start + structure-tools + catalog capabilities sections (capabilities base landed via [#19](https://github.com/oveddan/lx-mcp/pull/19); license: MIT, 2026-07-08)
