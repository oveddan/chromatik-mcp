...
- [~] **PR-7a** — Semantic catalog: format contract (`docs/catalog-format.md`), `lx-mcp-catalog` generation skill (hash-keyed incremental, Sonnet summarizers, portable sources config), initial 24 stock-LX pattern/effect entries in `package/src/main/resources/catalog/`, and the catalog-freshness step added to the dev loop — branch `pr-7a-semantic-catalog`. Docs are a cache of source understanding; three-tier runtime resolution (overlay → class's own jar → lx-mcp jar) so drop-in content jars can ship their own docs
- [~] **PR-7b** — `get_component_doc` MCP tool + `domain/Catalog.java` (three-tier lookup: `~/.lx-mcp/catalog/` overlay → class's own jar → absent; flat-frontmatter parse, bytecode-hash staleness false/true/"unknown" with per-class cache), `documented` flag in `list_available_*`, `CatalogFormatTest` gate (walks every entry inside `mvn package`) — branch `pr-7b-catalog-tool` (stacked on PR-7a)
