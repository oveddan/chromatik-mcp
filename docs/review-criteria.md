...
- **Typed domain result / wire-map boundary.** A stable domain result is a typed record or
  ...
  boundary. New or changed sibling tools emitting the same named object shape must reuse
  that serializer; test payload identity when the tools are read/write views of that same
  object or the documentation promises an identical shape. Map-shaped domain results are
  reserved for genuinely dynamic/open-ended data and must document that exception. Do not
  turn an unrelated PR into a wholesale migration of untouched legacy serializers. *Why:*
  compile-time field/type checking in the domain and one source of truth for the wire
  contract prevent the cross-tool drift demonstrated by `ParameterInfo` in calibration
  example 3.
