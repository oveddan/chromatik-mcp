...
| `move_channel` | `Mixer.DropChannel(LXAbstractChannel, int, LXGroup)` (LXCommand.java:1848-1888) | `ChannelMoveResult moveChannel(LX lx, String path, int toIndex)` | yes — assert plain-channel and whole-group-block order restored | The MCP index is an absolute post-removal destination. `DropChannel` instead expects a pre-removal flat-list coordinate for rightward group moves, so the primitive translates by the member count. Group membership is preserved: members cannot leave their group and top-level buses cannot split a group block. Bounds/membership are pre-validated so a rejected LX command cannot wipe history. |
...
| `list_modulations` | none (read-only) | `Modulators.listEnginePage(LX lx, LXModulationEngine engine, int offset, int limit)` | n/a | (Implemented PR-5c follow-up; paginated for #172.) Snapshots one engine's modulators plus a bounded page of compound/trigger wirings; scope arg shares `resolveEngine` with the mutation tools. |
