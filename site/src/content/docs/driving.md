...
read it here too. Installing it is one symlink: see
[Install the agent plugin](../plugin/).

The skill points at three bundled reference files for detail it deliberately keeps out
of always-loaded context —
[`addressing.md`](https://github.com/oveddan/chromatik-mcp/blob/main/agent-plugin/skills/driving-chromatik/references/addressing.md)
(canonical vs. OSC paths),
[`error-codes.md`](https://github.com/oveddan/chromatik-mcp/blob/main/agent-plugin/skills/driving-chromatik/references/error-codes.md)
(the full `Result` wire shape), and
[`composition.md`](https://github.com/oveddan/chromatik-mcp/blob/main/agent-plugin/skills/driving-chromatik/references/composition.md)
(the arrange timeline). They aren't reproduced on this site; read them from the plugin,
or from those links if you don't have it installed.
...
The 26 composition tools author the arrange timeline at `/lx/timeline/composition`
(most also accept a grid clip, `/lx/mixer/channel/N/clip/M`). Two rules govern all of
them:

- **Trust the echo.** Setters silently clamp. Every mutation payload carries the state
  read back from the engine — compare it against what you sent, and believe the payload.
- **Addresses are positional.** Lane paths `<clipPath>/lane/<n>` and event indices shift
  on any insert/remove/move, so re-list after every structural change rather than reusing
  a held address. Locators are the exception that catches people out: they are 1-indexed
  everywhere, while lane and event indices are 0-based.

Timeline positions travel as cursor objects — reads emit
`{millis, beatCount, beatBasis, formatted}`; writes take exactly one of `{millis}`,
`{beatCount[, beatBasis]}`, `{bars[, beats, sixteenths]}` (1-indexed), or
`{at: <origin>[, offsetBeats | offsetMillis]}`.

Before authoring a timeline, read the plugin's bundled `references/composition.md` — it
covers transport and markers, locators, lane lifecycle, reading and inserting automation,
editing and removing points and ranges, and audio/notes/record-arm, with the per-tool
clamping and undo behavior each of those carries.
